...
function backDb($host, $user, $pass, $dbname, $tables = '*'){
    ...
    foreach ($tables as $table) {
        ...
        $query = $conn->query($sql); //consulta de mostrar toda la tabla
        
        $columnCount = $query->field_count; //Contar columnas

        // Prepare SQLscript for dumping data for each table
        for ($i = 0; $i < $columnCount; $i ++) {
            while ($row = $query->fetch_row()) {    //Contiene una fila de la tabla
